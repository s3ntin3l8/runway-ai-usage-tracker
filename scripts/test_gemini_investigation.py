...
def print_bucket_comparison(buckets, label):
    ...
    for b in buckets:
        ...
        print(f"    Remaining Fraction: {fraction} ({int(fraction * 100)}% remaining = {int((1-fraction) * 100)}% used)")
        print(f"    Reset: {reset}")
...
async def compare_all_models():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if expiry < now:
            ...
            print(f"{GREEN}Token is valid (expires in {int((expiry - now)/1000/60)} minutes){RESET}")
        ...
        if discovered_project:
            buckets_discovered = await test_quota_with_project(
                client, headers, discovered_project, 
            ...
            for proj in gemini_projects[:3]:  # Test first 3
                ...
                if proj_id != discovered_project:
                    buckets = await test_quota_with_project(
                        client, headers, proj_id,
                        f"Alternative project ('{proj_id}')"
                    )
        ...
        if gemini3_models:
            ...
            print(f"  3. gemini-3 quotas are tracked differently (not in retrieveUserQuota)")
