...
def debug_chrome_extraction():
    ...
    if sys_os == "Darwin":
        ...
        if base_dir.exists():
            ...
            for profile_dir in base_dir.glob("Default"):
                all_targets.append(("Chrome", profile_dir))
            for profile_dir in base_dir.glob("Profile *"):
                all_targets.append(("Chrome", profile_dir))

        edge_dir = home / "Library/Application Support/Microsoft Edge"
        if edge_dir.exists():
            for profile_dir in edge_dir.glob("Default"):
                all_targets.append(("Edge", profile_dir))
            for profile_dir in edge_dir.glob("Profile *"):
                all_targets.append(("Edge", profile_dir))
    ...
    for browser_name, profile_dir in all_targets:
            ...
            try:
                with open(local_state_path) as f:
                    ...
                    if not has_crypt and sys_os == "Darwin":
                        logger.warning(
                            "⚠️  No 'os_crypt' in Local State — App-Bound Encryption (ABE) likely active"
                        )
                        abe_suspected = True
            except Exception:
                pass
        ...
        try:
            ...
            cursor.execute(
                "SELECT host_key, name, encrypted_value FROM cookies WHERE encrypted_value IS NOT NULL LIMIT 5"
            )
            ...
            for host_key, name, encrypted_value in rows:
                ...
                if encrypted_value[:3] == b"v20":
                    abe_confirmed = True
                logger.debug(
                    f"Attempting to decrypt cookie '{name}' (prefix: {prefix!r}) for domain '{host_key}'..."
                )
                ...
                if decrypted:
                    logger.info(
                        f"✅ SUCCESS: Successfully decrypted cookie '{name}' (prefix: {prefix!r}) for '{host_key}'"
                    )
                    decryption_success = True
                    break
                logger.debug(
                    f"Failed to decrypt cookie '{name}' (prefix: {prefix!r}) for '{host_key}'"
                )
            ...
            if decryption_success:
                ...
                if abe_confirmed:
                    logger.error(
                        "   Cause: v20 prefix detected — App-Bound Encryption (ABE) is ACTIVE"
                    )
                elif abe_suspected:
                    logger.error(
                        "   Cause: ABE suspected (no os_crypt in Local State, standard key derivation blocked)"
                    )
                else:
                    logger.error(
                        "   Cause: Unknown — Keychain key may be wrong or cookies use a different format"
                    )
    ...
    if success_count == 0:
        logger.error(
            f"Check Complete. Successfully verified {success_count}/{len(all_targets)} profiles."
        )
...
def _print_abe_workaround(sys_os: str):
    ...
    if sys_os == "Darwin":
        ...
        logger.warning(
            "     Log into claude.ai / opencode.ai in Safari and Runway will find the cookies."
        )
        ...
        logger.warning(
            "     Log into claude.ai / opencode.ai in Firefox and Runway will find the cookies."
        )
        logger.warning("  2. Use environment variables instead:")
