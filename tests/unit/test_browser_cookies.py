...
class TestSafariParser:
    def test_parses_binary_cookies_header(self):
        ...
        cookie_data = (b'A'*16 + struct.pack('<I', 32) + struct.pack('<I', 44) + 
                       struct.pack('<I', 52) + struct.pack('<I', 54) + 
                       b'example.com\x00' + b'session\x00' + b'/\x00' + b'xyz123\x00')
        
        full_page = page_data + cookie_data
        
        with patch('builtins.open', MagicMock()) as mock_open:
            mock_open.return_value.__enter__.return_value.read.side_effect = [
                b'cook', struct.pack('>I', 1), struct.pack('>I', len(full_page)), full_page
            ]
