...
class TestCardMerge:
    ...
    def test_fresh_quota_clears_sticky_collection_failing_flag(self):
        # Partial dict (documented merge_card_json API): fresh quota omits the
        # collection_failing key entirely — the recovery pop must drop residual
        # collection_failing=True.
        existing = json.dumps({"pct_used": 0.0, "collection_failing": True, "health": "good"})
        incoming = {"pct_used": 42.0, "health": "good"}
        parsed = json.loads(merge_card_json(existing, incoming))
        assert "collection_failing" not in parsed
        assert parsed["pct_used"] == 42.0

    def test_fresh_quota_overwrites_collection_failing_false_from_model_dump(self):
        # Real upsert path: LimitCard.model_dump(exclude_none=True) always
        # emits collection_failing=False (bool default is not None) — the
        # merge loop clears residual collection_failing even without the
        # recovery pop.
        from app.models.schemas import LimitCard

        existing = json.dumps({"pct_used": 0.0, "collection_failing": True, "health": "good"})
        incoming = LimitCard(
            service_name="Test", pct_used=42.0, health="good", collection_failing=False
        ).model_dump(exclude_none=True)
        assert incoming["collection_failing"] is False
        parsed = json.loads(merge_card_json(existing, incoming))
        assert parsed["collection_failing"] is False
        assert parsed["pct_used"] == 42.0
