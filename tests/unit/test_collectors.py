...
class TestOpenCodeCollector:
    """Test suite for OpenCode collector (api/web strategies + identity pinning).

    Most of the opencode tests live in
    ``tests/unit/test_opencode_account_id_fix.py``; this module retains
    the base-class enrichment-merge coverage.
    """
...
class TestOllamaApiCollector:
    """Ollama bearer `GET /api/usage` path (primary when an API key is present)."""

    def _usage_body(self, usage: float = 0.002) -> dict:
        return {
            "activity": {
                "cost": "0.00000",
                "period": {
            ...
            "limits": {
                "monthly": {
                    "usage": usage,
                    "models": [{"name": "web search", "request_count": 1}],
                }
            },
        }

    def _json_response(self, body: dict, status_code: int = 200) -> httpx.Response:
        resp = MagicMock(spec=httpx.Response)
    ...
    async def test_api_strategy_emits_monthly_card(self):
        """Bearer fetch returns one monthly card with the right window_type."""
        ...
        async def fake_get(provider, account_id=None):
            return ({"api_key": "test-key"}, {"source": "config"})

        async def fake_get_token(provider, token_type, account_id=None):
            return "test-key" if token_type == "api_key" else None
        ...
        with (
            ...
            patch(
                ...
                new_callable=AsyncMock,
                return_value=self._json_response(self._usage_body(usage=0.18)),
            ) as mock_http,
        ):
        ...
        assert card["provider_id"] == "ollama"
        assert card["data_source"] == OllamaCollector.DATA_SOURCE_API
        assert card["tier"] == "cloud"
        # Bearer header was set on the GET
        assert mock_http.await_args.kwargs["headers"]["Authorization"] == "Bearer test-key"

    @pytest.mark.asyncio
    async def test_api_strategy_no_key_returns_empty_for_fallback(self):
        """Without an API key the api strategy returns [] so the base
        collector falls through to the web (cookie) strategy."""
        from app.services.collectors.ollama import OllamaCollector
    ...
    async def test_api_strategy_401_sets_invalid_api_key(self):
        ...
        async def fake_get_token(provider, token_type, account_id=None):
            return "bad-key" if token_type == "api_key" else None
    ...
    def test_build_cards_from_api_usage_high_count_unit_is_token(self):
        """Absolute counts (usage > 1) surface as unit_type=token, not percent."""
        from app.services.collectors.ollama import OllamaCollector

        collector = OllamaCollector(account_id="acc_test")
        cards = collector._build_cards_from_api_usage(self._usage_body(usage=42.5))
        assert len(cards) == 1
        assert cards[0]["unit_type"] == "token"
        assert cards[0]["is_unlimited"] is False
