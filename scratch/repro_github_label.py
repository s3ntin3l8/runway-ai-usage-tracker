...
async def test_github_label_override():
    ...
    with patch.object(GitHubCollector, "_get_token", return_value="fake_token"):
        ...
        async def mock_request(*args, **kwargs):
            url = args[1] if len(args) > 1 else (args[0] if isinstance(args[0], str) else kwargs.get("url"))
            ...
            if "/user" in url and "/user/emails" not in url:
                return MockResponse({"login": "bjoern", "name": "Björn Hansen", "email": "real@email.com"})
            if "/user/emails" in url:
                return MockResponse([{"email": "real@email.com", "primary": True, "verified": True}])
            return MockResponse({}, 404)

        with patch("app.services.collectors.github.http_request_with_retry", side_effect=mock_request):
            with patch("app.services.token_cache.token_cache.update_account_metadata", return_value=None):
                ...
                if results:
                    for card in results:
                        print(f"Card: {card['service_name']} | Label: {card.get('account_label')} | Detail: {card['detail']}")
                        ...
                        assert "real@email.com" not in card["detail"], f"Email found in detail: {card['detail']}"
                else:
