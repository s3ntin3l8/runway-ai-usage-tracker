...
def get_keychain_secret(
    service: str, account: str | None = None, force_refresh: bool = False
) -> str | None:
    """
    ...
    try:
        ...
        if duration_ms > 1000:
            logger.info(
                f"⚠️ Slow Keychain query: {duration_ms}ms (Likely user interaction or timeout)"
            )
        ...
        if "The specified item could not be found" in err:
            ...
            logger.warning(
                f"❌ Keychain access denied: User interaction not allowed for '{service}'."
            )
        else:
            # Assume any other failure is a user denial or permission issue
            logger.warning(
                f"❌ Keychain access denied or failed for {service} (Code {result.returncode})"
            )
