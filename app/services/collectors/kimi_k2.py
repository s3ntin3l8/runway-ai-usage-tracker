...
class KimiK2Collector(BaseCollector):
    ...
    async def _get_current_key(self) -> str | None:
        ...
        db_key = credential_provider.get_provider_api_key("kimi_k2", account_id=account_id)
        if db_key:
            self._current_input_source = "config"
            return db_key
        from app.services.token_cache import token_cache

        cache_data = await token_cache.get_with_metadata("kimi_k2", account_id=account_id)
        if cache_data:
            ...
            if cached_key:
                return cached_key
        key = (settings.KIMI_K2_API_KEY or None) if account_id == "default" else None
        if key:
