...
**Ingest path:** Sidecar batches up to 1000 events per push to `POST /api/v1/fleet/ingest` (HMAC-signed, rate-limited to 600/min per source IP). Server runs `EventIngestor`, which deduplicates by `(provider_id, event_id)` (re-attributing same-sidecar account changes), computes cost via `cost_calculator`, updates rollups, and triggers `window_closer._maybe_close_previous_window` on quota-window boundaries.
...
**Account identity:** `app/services/account_identity.py:resolve_account_id` — email > UUID > SHA256 hash > `"default"`. Sidecar identity is the hostname; never part of unique constraints on the canonical (`provider_id`, `account_id`) pair. Every write path (cards, `EventIngestor`, token-cache keys, provider-config PUT) stores ids via `canonical_account_id` (emails lowercased, opaque ids verbatim); the sidecar mirrors it in `scripts/sidecar_pkg/identity.py`, and `app/services/account_canonicalization.py` repairs legacy rows at startup.
